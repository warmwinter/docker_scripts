...
$CMD run --detach --interactive --name $IMAGEREPO-$IMAGETAG -e MODE=standalone -e MYSQL_SERVICE_HOST=${LOCAL_IP} -e MYSQL_SERVICE_USER="dbadmin" -e MYSQL_SERVICE_PASSWORD="dbAdmin6yhn&UJM" --privileged --publish ${LISTEN_PORT}:8848  --publish 7848:7848 --publish 9848:9848 --publish 9849:9849 --restart unless-stopped --tty --volume /web/wwwroot:/web/wwwroot:rw --volume /web/docker/logs/${IMAGEREPO}:/home/nacos/logs:rw $IMAGEREPO:$IMAGETAG  
